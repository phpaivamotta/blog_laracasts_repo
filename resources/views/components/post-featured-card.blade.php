    ...
    <div class="py-6 px-5 lg:flex">
        ...
        <div class="flex-1 flex flex-col justify-between">
            <header class="mt-8 lg:mt-0">
                ...
                <div class="mt-4">
                    <h1 class="text-5xl font-bold lg:font-normal lg:text-3xl">
                        <a href="/posts/{{$post->slug}}" class="transition-colors duration-300 hover:text-blue-500">
                            {{ $post->title }}
                        </a>
                    </h1>

                    <span class="mt-2 block text-gray-400 text-lg lg:text-xs">
                        Publicado <time>{{$post->created_at->diffForHumans()}}</time>
                    </span>
                </div>
            </header>

            <div class="text-4xl lg:text-sm mt-6 lg:mt-2 space-y-4">
                {!! $post->excerpt !!}
            </div>
            ...
            <footer class="flex justify-between items-center mt-8">
                <div class="flex items-center text-sm">
                    ...
                    <!-- Likes -->
                    <svg viewBox="-10 -10 40 40" class="w-20 lg:w-9">
                    ...
                    <div class="ml-2 mr-2">
                        <h5 class="text-2xl lg:text-sm text-gray-700 font-semibold">{{ $post->likeCount  }}</h5>
                    </div>

                    <!-- comments -->
                    <svg viewBox="-10 -10 40 40" version="1.1" class="w-20 lg:w-9">
                    ...
                    <div class="ml-2">
                        <h5 class="text-2xl lg:text-sm text-gray-700 font-semibold">{{ $post->comment->count() }}</h5>
                    </div>
